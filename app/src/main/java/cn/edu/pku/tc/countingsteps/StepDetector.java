...
public class StepDetector implements SensorEventListener {
    ...
    private int lastStep=-1;  //记录上一次临时的步数

    public static float average=0;  //用x,y,z轴三个维度算出平均值

    private Timer timer;

    private long duration=3500;  //倒计时3.5秒，3.5秒内不会显示计步，用于屏蔽细微波动
    ...
    private void detectorNewStep(float values) {
        if(gravityOld == 0){
            ...
            if(DetectorPeak(values, gravityOld)){ //检测是否是波峰
    ...
    private void preStep(){
        if(CountTimeState == 0){
            ...
            time = new TimeCount(duration, 700); //倒计时3.5秒，每0.7秒去检测一次
            time.start();
            CountTimeState = 1;  //计时3.5秒结束后，将CountTimeState改为1，即计时中
            Log.v(TAG, "开启计时器");
        }else if(CountTimeState == 1){
            TEMP_STEP++;          //如果传感器测得的数据满足走一步的条件则步数加1
            Log.v(TAG,"计步中 TEMP_STEP:"+TEMP_STEP);
        }else if(CountTimeState == 2){
            CURRENT_STEP++;  //在计步状态中，直接把步数加1
